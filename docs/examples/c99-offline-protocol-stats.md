# C99 Example: Offline Protocol Statistics

This example reads a `.pcap` file and counts frame types using `zpcap_loop`.

## Source (`examples/08_offline_protocol_stats.c`)
...
```bash
zig build
gcc -std=c99 -o docs/examples/c99-offline-protocol-stats examples/08_offline_protocol_stats.c -Iinclude -Lzig-out/lib -lzcap
LD_LIBRARY_PATH=zig-out/lib ./docs/examples/c99-offline-protocol-stats
```

## What this shows

- Offline processing with `zpcap_open_offline` + `zpcap_loop`.
- Protocol counts with zero-copy callback access to packet data.
- Optional VLAN-aware Ethernet type handling.

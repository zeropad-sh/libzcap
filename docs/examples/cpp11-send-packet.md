# C++11 Example: Packet Injection APIs

This example shows both injection entry points:
`zpcap_sendpacket` and `zpcap_send`.

## Source (`examples/17_send_packet.cpp`)
...
```bash
zig build
g++ -std=c++11 -o docs/examples/cpp11-send-packet examples/17_send_packet.cpp -Iinclude -Lzig-out/lib -lzcap
LD_LIBRARY_PATH=zig-out/lib ./docs/examples/cpp11-send-packet lo
```
